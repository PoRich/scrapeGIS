 ...
 * @returns {object}
 */
function zipObjectEqualLength(labelsArray, dataArray) {
  // returns object
  var p = {}
...
module.exports = {
  ...
  async getTargetState(site) {
    ...
    if (site === "yp") {
      ...
      throw new UserException(
        "unrecognized site; HINT: must be yelp, yp, loopnet or bcbs"
      )
    }
    try {
      let res = await db.query(
        "select array(select distinct state_abbrev \
                from biz_data.dental_meta where $1 is null) \
                as target;",
        [colName]
      )
  ...
  async getTargetCity(state, site) {
    ...
    try {
      let res = await db.query(
        "select regexp_split_to_array((select concat_ws(';', city, state_abbrev) \
            from biz_data.dental_meta\
            where ($1 <> $2 or $2 is null) and \
            state_abbrev=$3 limit 1), ';') as target;",
        [status, max, state]
      )
      console.log(
        `###### Retrieved target city ${res["rows"][0]["target"]} for ${site} scraper ######`
      )
      return res["rows"][0]["target"]
    } catch (e) {
      throw e
  ...
  async updateMetaStatus(_currentPage, _totalPages, _target, site) {
    ...
    console.log(
      `updateMetaStatus called with _currentPage ${_currentPage}, _totalPages ${_totalPages}, _target ${JSON.stringify(
        _target
      )}, site ${site}`
    )
    let queryText = ""
    if (site == "yelp") {
      queryText =
        "update biz_data.dental_meta set (yelp_status, yelp_max_pages) = ($1, $2) \
                               where state_abbrev=upper($3) and city=$4"
    } else if (site == "yp") {
      queryText =
        "update biz_data.dental_meta set (yp_status, yp_max_pages) = ($1, $2) \
                               where state_abbrev=upper($3) and city=$4"
    } else if (site == "loopnet_sale") {
      queryText =
        "update biz_data.dental_meta set (loopnet_sale_status, loopnet_sale_max_pages) = ($1, $2) \
                               where state_abbrev=upper($3) and city=$4"
    } else if (site == "loopnet_lease") {
      queryText =
        "update biz_data.dental_meta set (loopnet_lease_status, loopnet_lease_max_pages) = ($1, $2) \
                               where state_abbrev=upper($3) and city=$4"
    }
    ...
    try {
      ...
      console.log(
        ` ######## Updated biz_data.dental_meta ${site} = ${_currentPage} for ${_target["city"]}, ${_target["state"]} ########`
      )
      return
    } catch (e) {
  ...
  async prepPage(
    ...
    try {
      ...
      if (badUrlCss) {
        // Try to test if page is no longer active
        try {
    ...
    try {
      // Wait 5 seconds or target css selector to load
      ...
      try {
        // TODO - get recaptcha css selector
  ...
  zipObject(labelsArray, dataArray) {
    // when data array is longer and labels should be repeated
    var payload = []
    var p = {}
