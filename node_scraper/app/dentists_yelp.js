...
async function initial_scrape(targetState) {
  ...
  while (_target) {
    ...
    console.log(
      `========= SCRAPING YELP city: ${target["city"]}, state ${target["state"]} ==============`
    )
    var totalPages = 1

    for (let j = 1; j <= totalPages; j = j + 1) {
      url = `https://www.yelp.com/search?find_desc=Dentists&find_loc=${
        target["city"]
      }%2C+${target["state"]}&start=${(j - 1) * 10}`
...
async function detail_scrape(target, page) {
  ...
  while (_targetList.length > 0) {
    ...
    console.log(
      `saved ${addrPayload[0]} ${addrPayload[1]} --> d_id: ${targetObj["d_id"]}`
    )
  }
  ...
  const payload = await page.evaluate(() => {
    var parentElement = Array.from(
      document.querySelectorAll(
        'div[class=" scrollablePhotos__09f24__1PpB8 arrange__09f24__2v3uJ border-color--default__09f24__1eOdn"]'
      )
    )
    ...
    var pre = parentElement.map(function (e, i) {
      return {
        ...
        specialty: e.firstElementChild.querySelectorAll(
          'p[class=" css-n6i4z7"]'
        ), // [0].innerText,
        rating: e.firstElementChild.querySelectorAll(
          'div[class=" attribute__09f24__3znwq display--inline-block__09f24__3L1EB margin-r1__09f24__BCulR border-color--default__09f24__1eOdn"]'
        ),
        numRatings: e.firstElementChild.querySelectorAll(
          'span[class="reviewCount__09f24__EUXPN css-e81eai"]'
        ), // [0].innerText,
    ...
    var bizPayload = pre.map((e, i) => {
      ...
      return {
        ...
        rating:
          e.rating.length == 1
            ? Number(rating_regex.exec(e.rating[0].innerHTML)[1])
            : null,
        numRatings:
          e.numRatings.length == 1 ? Number(e.numRatings[0].innerText) : null,
        phone:
          e.phone.length >= 1
            ? phone_regex.exec(e.phone[0].innerText)
              ? e.phone[0].innerText
              : null
            : null,
        addr:
          e.addr.length >= 2
            ? phone_regex.exec(e.addr[1].innerText)
              ? null
              : e.addr[1].innerText
            : null,
        district:
          e.addr.length == 3
            ? phone_regex.exec(e.addr[2].innerText)
              ? null
              : e.addr[2].innerText
            : null,
      }
    ...
    var pagination = document.querySelectorAll(
      'div[class=" border-color--default__09f24__1eOdn text-align--center__09f24__1P1jK"] > span[class=" css-e81eai"]'
    )[0].innerHTML
...
async function scrapeAddress(pageURL, page) {
  ...
  try {
    await page.waitForSelector('p[class=" css-m6anxm"] > span', {
      timeout: 30000,
    })
  } catch (e) {
  ...
  const payload = await page.evaluate(() => {
    var fullAddr = document.querySelector(
      'p[class=" css-1h1j0y3"] > p[class=" css-e81eai"]'
    )
      ? document.querySelector(
          'p[class=" css-1h1j0y3"] > p[class=" css-e81eai"]'
        ).innerText
      : -1
    var district = document.querySelector('p[class=" css-m6anxm"] > span')
      ? document.querySelector('p[class=" css-m6anxm"] > span').innerText
      : -1

    var sideBox = document.querySelectorAll(
      'div[class=" arrange-unit__373c0__1piwO arrange-unit-fill__373c0__17z0h border-color--default__373c0__2oFDT"] >p[class=" css-1h1j0y3"]'
    )
      ? document.querySelectorAll(
          'div[class=" arrange-unit__373c0__1piwO arrange-unit-fill__373c0__17z0h border-color--default__373c0__2oFDT"] >p[class=" css-1h1j0y3"]'
        )
    ...
    // geocode
    var mapSrc = document
      .querySelectorAll('img[alt="Map"]')[0]
      .getAttribute("srcset")
...
async function saveBiz(payload, _target, url) {
  try {
    ...
    const queryText =
      "INSERT INTO biz_data.dental_yelp(biz_name, specialty, phone, addr, \
...
async function getProfileLinks(target) {
  try {
    const queryText =
      "select d_id, profile_url from dental_data.yelp where addr is null and state_abbrev=$1 and target_city=$2 order by d_id"
...
async function saveFullAddr(_d_id, _addrPayload) {
  try {
    await db.query("BEGIN")
    const queryText =
      "update dental_data.yelp set addr = $1, district = $2, \
...
*/
  try {
    const queryText =
      "select array(select url from biz.dentists_redo where u_status is null order by url) as url"
    var res = await db.query(queryText)
...
async function geocodePostFacto(page) {
  ...
  const urlsQuery = await db.query(
    "with profile_urls as (select profile_url from dental_data.yelp \
        where the_geom is null and no_geocode is null order by 1 asc) \
        select array_agg(profile_url) urls from profile_urls"
  ) // other instance is desc order
  ...
  while (urls.length > 0) {
    ...
    if (payload !== -1) {
      ...
      try {
        ...
        await db.query("COMMIT")
        console.log(
          `geocodePostFacto: Saved geocode for d_id: ${r["rows"][0]["d_id"]}} -> dental_data.yelp`
        )
      } catch (e) {
      ...
      try {
        ...
        const r = await db.query(
          "update dental_data.yelp set no_geocode=1 where profile_url=$1 returning *",
          [url]
        )
        await db.query("COMMIT")
        console.log(
          `geocodePostFacto: Set no_geocde for: ${r["rows"][0]["profile_url"]}} -> dental_data.yelp`
        )
      } catch (e) {
        await db.query("ROLLBACK")
        console.log(
          `geocodePostFacto: Failed to save no_geocode on ${url}: ${e}`
        )
        throw e
      }
    }
...
async function scrapeGeom(pageURL, page) {
  ...
  const r = await ScrapeTools.prepPage(
    pageURL,
    page,
    scrapeGeom,
    _waitForCss,
