...
async function initial_scrape(targetState) {
  ...
  while (_target) {
    ...
    console.log(
      `========= SCRAPING YELLOW PAGES city: ${target["city"]}, state ${target["state"]} ==============`
    )
    let pageNum = 1
    let nextPage = true
    ...
    while (nextPage) {
      console.log(
        `***************** SCRAPING PAGE # ${pageNum} of ${target["city"]}, ${target["state"]} ***************** `
      )
      ...
      if (bizData == -1 || !bizData || bizData.length == 0) {
        // if no search results
        await ScrapeTools.updateMetaStatus(-1, -1, target, "yp")
        console.log(
          `No search results for ${target["city"]}-${target["state"]}`
        )
        break
      } else {
        ...
        for (let i = 0; i < bizData.length; i = i + 1) {
          ...
          relevantResults = bizData[i]["profile_url"].includes(localitySearch)
            ? relevantResults + 1
            : relevantResults
        }
        console.log(
          `================ ${relevantResults} Relevant Results on page ================`
        )
        // update meta tracker
        ...
        nextPage =
          (relevantResults > 0 && bizData.length > 7) || pageNum <= 5
            ? true
            : false
      }
    }
...
async function scrapeYP(pageURL, page) {
  ...
  const payload = await page.evaluate(() => {
    var parentElement = Array.from(
      document.querySelectorAll('div[class="info"]')
    )
    // Gather desired elements
    var bizPayload = parentElement.map(function (e, i) {
      try {
        let currentYear = new Date().getFullYear()
        const street_addr = e.querySelectorAll('div[class="street-address"]')[0]
          ?.innerText
        const locality = e.querySelectorAll('div[class="locality"]')[0]
          ?.innerText
        let scraped_data = {
          // preprocess
          biz_name: e.querySelectorAll('a[class="business-name"]')[0]
            ?.innerText,
          specialty: e.querySelectorAll('div[class="categories"]')[0]
            ?.innerText,
          yearEst: e.querySelectorAll(
          ...
          rating: e
            .querySelectorAll('span[class="count"]')[0]
            ?.parentElement.className.replace("result-rating ", "")
            .trim(),
          numRatings: e.querySelectorAll('span[class="count"]')[0]?.innerText,
          website:
            e.querySelector('div[class="links"]')?.firstElementChild?.href,
          phone: e.querySelectorAll(".phone")[0]?.innerText,
          full_addr: e.querySelectorAll('p[class="adr"]')[0]?.innerText
            ? e.querySelectorAll('p[class="adr"]')[0]?.innerText
            : street_addr + ", " + locality,
          st_addr: street_addr,
          locality: locality,
        }
        return scraped_data
...
async function saveBizYP(payload, _target, url) {
  try {
    ...
    const queryText =
      "INSERT INTO biz_data.dental_ypages(biz_name, specialty, year_est, rating, num_reviews, \
...
async function geocodePostFacto(page) {
  ...
  const urlsQuery = await db.query(
    "with profile_urls as (select profile_url from dental_data.ypages \
  ...
  while (profileURLs.length > 0) {
    ...
    if (url === -1) {
      console.log(
        `************ unable to get DirectionURL for ${profileURL} ************`
      )
      db.query(
        "update dental_data.ypages set no_geocode=1 where profile_url=$1",
        [profileURL]
      )
      continue
    }
    ...
    if (payload !== -1) {
      ...
      try {
        ...
        await db.query("COMMIT")
        console.log(
          `geocodePostFacto: Saved geocode for d_id: ${r["rows"][0]["d_id"]}} -> dental_data.yp`
        )
      } catch (e) {
        await db.query("ROLLBACK")
        console.log(
          `geocodePostFacto: Failed to save geom to dental_data.yp: ${e}`
        )
        throw e
      }
    } else {
      try {
        ...
        console.log(
          `geocodePostFacto: Set no_geocde for: ${profileURL} -> dental_data.yp`
        )
        await db.query("COMMIT")
      } catch (e) {
        await db.query("ROLLBACK")
        console.log(
          `geocodePostFacto: Failed to save no_geocode on ${profileURL} to dental_data.yp: ${e}`
        )
        throw e
      }
    }
