...
const proxyUrls = ScrapeTools.proxies.map(
    (e) => `http://${process.env.PROXY_USER}:${process.env.PROXY_PASSWORD}@${e}`
)
...
async function runTests(page) {
    ...
    await page.screenshot({
        path: "./screenshots/apify-bot-test.png",
        fullPage: true,
    })
    ...
    await page.screenshot({
        path: "./screenshots/apify-ip-test.png",
        fullPage: true,
    })
...
async function loopnetSearch(page) {
    ...
    try {
        ...
        const closePopUp =
            'button[class="csgp-modal-close ln-icon-close-hollow"]'
    ...
    try {
        ...
        await page.screenshot({
            path: `./screenshots/apify-${target.city}-${target.state}-loopnet-search.png`,
            fullPage: true,
        })
        ...
        await page.screenshot({
            path: `./screenshots/apify-${target.city}-${target.state}-loopnet-search-err.png`,
            fullPage: true,
        })
...
async function saveMapPin(d) {
    try {
        // href is temporarily set to loopnet_id because it cannot be null
        const queryText =
            "insert into real_estate.loopnet (loopnet_id, the_geom, href, target_plcidfp, listing_type, date_scraped) \
...
async function saveBroker(broker) {
    ...
    try {
        ...
        client.release()
        console.log(
            `ERROR saveJsonLD (saveBroker: ${
                broker.name
            } | db response ${JSON.stringify(
                a1Query
            )}) - error saving data: ${e}`
        )
        throw e
...
async function saveBrokerage(broker) {
    ...
    try {
        ...
        client.release()
        console.log(
            `ERROR saveJsonLD (Broker Organization: ${
                broker.worksFor.name
            } | db result ${JSON.stringify(a2Query)}) - error saving data: ${e}`
        )
        throw e
    }
}
...
async function saveJsonLD(d) {
    ...
    try {
        ...
        if (d["availableAtOrFrom"]) {
            if (
                (d.streetAddress =
                    d.availableAtOrFrom.address["@type"] === "PostalAddress")
            ) {
        ...
        if (d.offeredBy) {
            ...
            let brokerArray =
                Array.from(d.offeredBy).length > 0
                    ? Array.from(d.offeredBy)
                    : [d.offeredBy] // Array.from will return null if only one object
                ...
                if (broker["@type"] === "Person") {
                    ...
                    if (broker.worksFor) {
                        if (broker.worksFor["@type"] === "Organization") {
                            ...
                            console.log(
                                `NOTICE: broker doesn't work for organization [NOT SAVED]: ${broker.worksFor["@type"]}`
                            )
                        }
                    }
                    ...
                    if (agent1_id && agent2_id) {
                        ...
                        while (!aAgent_id) {
                            // Save broker-brokerage relationship to agent.a_agent table
                            aAgent_id = await saveAgentRelationship(
                                agent1_id,
                                agent2_id
                            )
                            count += 1
                        ...
                        if (aAgent_id) {
                            ...
                            while (!updatedField) {
                                ...
                                queryResult =  await db.query('update real_estate.loopnet set listing_broker=$1 \
                                ...
                                */
                                ...
                                if (idx === 0) {
                                    ...
                                    queryResult2 = await db.query(
                                        "update real_estate.loopnet set (listing_broker, listing_brokers) = ($1, Array[$2::INT])  \
                                    ...
                                    queryResult = await db.query(
                                        "update real_estate.loopnet set listing_brokers = array_append(listing_brokers, $1::INT) \
                                ...
                                if (queryResult?.row) {
                                    updatedField =
                                        queryResult["row"][0]["listing_broker"]
                                }
                                // await db.query('COMMIT');
                        ...
                        console.log(
                            `NOTICE: NO BROKER RELATIONSHIP SAVED (one agent_id is missing) agent1_id: ${agent1_id} | agent2_id: ${agent2_id}`
                        )
                    }
                } else {
                    console.log(
                        `NOTICE: broker is not a person [NOT SAVED]: ${broker["@type"]}`
                    )
                }
            })
        }
        ...
        const queryText =
            "update real_estate.loopnet set (raw_jsonld, href, price, category, imgs, listing_desc, date_scraped) = \
        ...
        await db.query("COMMIT")
        console.log(
            `saveJsonLD db result: ${JSON.stringify(
                queryResult["rows"][0],
                null,
                "\t"
            )}`
        ...
        console.log(
            `saveJsonLD saved loopnet_id: ${JSON.stringify(
                queryResult["rows"][0]["loopnet_id"]
            )} - ${JSON.stringify(
                queryResult["rows"][0]["raw_jsonld"]["streetAddress"]
            )}`
        )
    } catch (e) {
        await db.query("ROLLBACK")
        console.log(
            `ERROR (saveJsonLD error saving loopnet_id ${loopnet_id}): ${e}`
        )
        throw e
    }
}
...
async function saveArticles(d) {
    try {
        ...
        let sqft = null
        if (d._sqft) {
            // do not delete d._sqft b/c it is an array
            // get the max sqft
        ...
        const queryText =
            "update real_estate.loopnet set (addr, imgs, blding_sqft, lot_acre, exec_sum, raw_article, cap_rate, date_scraped) = \
        ...
        await db.query("COMMIT")
        console.log(
            `saveArticles - saved loopnet_id: ${JSON.stringify(
                queryResult["rows"][0]["loopnet_id"]
            )} - ${JSON.stringify(queryResult["rows"][0]["addr"])}`
        )
    } catch (e) {
        await db.query("ROLLBACK")
        console.log(
            `ERROR (saveArticles loopnet_id: ${loopnet_id}) error: ${e}`
        )
        throw e
    }
}
...
Apify.main(async () => {
    ...
    await requestQueue.addRequest({
        url: `https://www.loopnet.com/search/commercial-real-estate/${target.city.toLowerCase()}-${target.state.toLowerCase()}/for-${listing_type}/`,
    })
    ...
    const crawler = new Apify.PuppeteerCrawler({
        ...
        handlePageFunction: async ({
            ...
            // Block Resources
            page = await ScrapeTools.blockResources(page, [
                "stylesheet, image, media, font, script, other",
            ])

            // await runTests(page);

            console.log(
                `Using proxy url ${proxyInfo.url} and sessionID ${proxyInfo.sessionId}`
            )
            ...
            const totalPagesCss =
                'div[class="total-results"] span[class="ng-binding"]'
            ...
            try {
                ...
                totalResultsPages = await page.$eval(totalPagesCss, (el) => {
                    console.log(
                        `totalResultsPages element ${JSON.stringify(el)}`
                    )
                    return el.innerText
                })
                await page.screenshot({
                    path: "./screenshots/loopnet-apify.png",
                    fullPage: true,
                })
                // await puppeteer.saveSnapshot(page); // saves screen shot to keyvalue store
            } catch (e) {
                // TODO - check errors for specific types of network errors
                ...
                await page.screenshot({
                    path: "./screenshots/loopnet-apify-err.png",
                    fullPage: true,
                })
            ...
            const blocked = await page.$$eval(
                "section.access-denied-wrapper h1",
                (el) => {
                    return el.innerText
                }
            )
            ...
            if (blocked === "Temporarily Blocked") {
                ...
                // automatically session.markGood()
                if (loopCounter === 1) {
                    // Run this block of code ONCE per target city/state
                    // Scrape map pins
                    const mapPins = await page.$$eval(
                        "div#mapState div",
                        ($pins) => {
                            const scrapedPins = []
                ...
                const rentRegex =
                    /\$(\d{1,}\.\d{2,2})[\s\-\$]*(\d{1,}\.\d{2,2})?\s\SF\/Yr/ // $35.00 - $37.00 SF/Yr
                // const sqftRegex = /((\d{0,3},)?\d{1,3})\sSF(?!\/Yr)/;
                if (jsonld) {
                    searchResultsJsonLD = jsonld.filter(
                        (e) => e["@type"] === "SearchResultsPage"
                    )
                    searchResultsJsonLD = searchResultsJsonLD
                        ? searchResultsJsonLD[0]["about"].map((e) => e.item)
                        : null
                    ...
                    searchResultsJsonLD.forEach((e) => {
                        ...
                        if (e.price) {
                            rentMatch =
                                rentRegex.exec(e.price) ||
                                rentRegex.exec(e.description)
                            if (e.price.includes("-")) {
                                e._prices = [
                                    Number(rentMatch[1]),
                                    Number(rentMatch[2]),
                            ...
                            e._avg_price =
                                ScrapeTools.sum(e._prices) / e._prices.length
                        }
                    })
                    // console.log(`jsonld ${JSON.stringify(searchResultsJsonLD, null,'\t')}`);
                }
                // Save JsonLD to db
                searchResultsJsonLD.forEach(async (d) => {
                    if (d) {
                        saveJsonLD(d)
                    }
                })
                ...
                const articles = await page.$$eval("article", ($articles) => {
                    ...
                    $articles.forEach(($a) => {
                            ...
                            addr: `${
                                [
                                    ...$a.getElementsByClassName(
                                        "placard-carousel-pseudo"
                                    ),
                                ][0].title
                            } ${$a.getAttribute("gtm-listing-zip")}`,
                            images: Array.from(
                                $a.getElementsByClassName("slide")
                            ).map((d) => d.children[0].children[0].content),
                            factSummary: [
                                ...$a.getElementsByClassName("data-points-a"),
                            ][0]
                                ? [
                                      ...$a.getElementsByClassName(
                                          "data-points-a"
                                ...
                                : null,
                            briefDesc: [
                                ...$a.getElementsByClassName("data-points-b"),
                            ][0]
                                ? [
                                      ...$a.getElementsByClassName(
                                          "data-points-b"
                        ...
                        let banner = [
                            ...$a.getElementsByClassName("right-h4"),
                        ][0]
                            ? [...$a.getElementsByClassName("right-h4")][0]
                                  .innerText
                            : null
                        if (banner) {
                        ...
                        let oz = $a.getElementsByClassName(
                            "tag tag-opportunity-zone"
                        )[0]
                            ? true
                            : false
                        if (oz) {
                ...
                const sqftRegexArticle =
                    /(\d{0,3},?\d{1,3})?(\s-\s)?(\d{0,3},?\d{1,3})\sSF(?!\/YR)/
                    ...
                    if (e.factSummary) {
                        ...
                        e.factSummary.forEach((d) => {
                            ...
                            if (sqftMatch) {
                                if (d.includes("-")) {
                                    e._sqft = [
                                        Number(sqftMatch[1].replace(",", "")),
                                        Number(sqftMatch[3].replace(",", "")),
                                    ]
                                } else {
                                    e._sqft = [
                                        Number(
                ...
                try {
                    nextPage = await page.$eval("a.caret-right-large", (el) => {
                        console.log(
                            `nextPageResults element ${JSON.stringify(el)}`
                        )
                        return el.href
                    })
                } catch (e) {
...
delete from real_estate.loopnet where target_plcidfp='4260000';
select * from real_estate.loopnet where target_plcidfp='4260000' and raw_jsonld is not null;
