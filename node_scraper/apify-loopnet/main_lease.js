...
async function saveBroker(broker) {
    ...
    try {
        ...
        console.log(
            `Attempt ${count}: saveJsonLD (Broker) - agent1_id: ${agent1_id}`
        )
        return agent1_id
    } catch (e) {
...
async function saveBrokerage(broker) {
    ...
    try {
        ...
        console.log(
            `Attempt: ${count}: saveJsonLD (Broker Organization) - agent2_id: ${agent2_id}`
        )
        return agent2_id
    } catch (e) {
        await client.query("ROLLBACK;")
        client.release()
        console.log(
            `saveJsonLD (Broker Organization: ${
                ...
                if (broker["@type"] === "Person") {
                    ...
                    if (broker.worksFor) {
                        if (broker.worksFor["@type"] === "Organization") {
                            ...
                            console.log(
                                `broker doesn't work for organization [NOT SAVED]: ${broker.worksFor["@type"]}`
                            )
                        }
                    }
                    ...
                    if (agent1_id && agent2_id) {
                        ...
                        while (!aAgent_id) {
                            ...
                            count += 1
                            console.log(
                                `Called saveAgentRelationship ${count} times - aAgent_id = ${aAgent_id}`
                            )
                        }
                        ...
                        if (aAgent_id) {
                            ...
                            console.log(
                                `290 - Attempt: ${listingBrokerUpdateCount} Updated loopnet_id: ${loopnet_id} to listing_broker: ${updatedField}`
                            )
                        }
                    } else {
                        console.log(
                    ...
                    console.log(
                        `NOTE: broker is not a person [NOT SAVED]: ${broker["@type"]}`
                    )
                }
            })
        }
...
async function saveArticles(d) {
    try {
        ...
        const queryText =
            "update real_estate.loopnet set (addr, imgs, blding_sqft, exec_sum, raw_article, date_scraped) = \
...
Apify.main(async () => {
    ...
    await requestQueue.addRequest({
        url: `https://www.loopnet.com/search/commercial-real-estate/${target.city.toLowerCase()}-${target.state.toLowerCase()}/for-lease/`,
    })
