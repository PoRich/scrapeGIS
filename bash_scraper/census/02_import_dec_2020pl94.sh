...
for i in `find ${DATA_DIR} -name *.zip`; do
    unzip -d ${DATA_DIR} $i
done
...
for i in `find ${DATA_DIR} -name *geo2020.pl`; do
    ...
    #cat $i | csvgrep -H -d -e utf-8 \| -c 3 -m 750 | psql -v ON_ERROR_STOP=1 -q -c "COPY dec2020_pl94.geoheader FROM STDIN WITH CSV HEADER ENCODING 'latin1';"
...
for i in $(find ${DATA_DIR}/ -name *12020.pl); do
    echo `basename $i`
...
for i in $(find ${DATA_DIR} -name *22020.pl); do
    echo `basename $i`
